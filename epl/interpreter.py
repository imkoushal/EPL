...
class Interpreter:
    ...
    def _exec_use_js(self, node: ast.UseJSStatement, env: 'Environment'):
        """Handle Use javascript/typescript statements."""
        if self.safe_mode:
            raise EPLRuntimeError(
                '"Use javascript" is not allowed in safe mode (--sandbox). '
                'JavaScript FFI provides unrestricted system access.',
                node.line,
            )
    ...
    def _eval_property_access(self, node: ast.PropertyAccess, env: Environment):
        ...
        # v5.3: JavaScript module property access
        if isinstance(obj, JSModule):
