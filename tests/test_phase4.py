...
def test_registry():
    ...
    # T1: Module imports
    try:
        from epl.registry import DownloadStats, GitHubRegistry, RegistryCache
    ...
    from epl.registry import registry_info, registry_publish, registry_search, registry_stats
...
def test_doc_generator():
    ...
    # T1: Module imports
    try:
        from epl.doc_linter import DocEntry, DocGenerator, DocParam, ModuleDoc
...
def test_50_packages():
    ...
    check('51 packages in registry', len(BUILTIN_REGISTRY) == 51, f'got {len(BUILTIN_REGISTRY)}')
    ...
    packages_with_keywords = sum(1 for info in BUILTIN_REGISTRY.values() if 'keywords' in info)
...
def test_site_generator():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        check('reference.html exists', os.path.isfile(os.path.join(tmpdir, 'reference.html')))

        # T8: api.html exists
        check('api.html exists', os.path.isfile(os.path.join(tmpdir, 'api.html')))

        # T9: packages.html exists
        check('packages.html exists', os.path.isfile(os.path.join(tmpdir, 'packages.html')))

        # T10: tutorials.html exists
        check('tutorials.html exists', os.path.isfile(os.path.join(tmpdir, 'tutorials.html')))

        # T11: compiler.html exists
        check('compiler.html exists', os.path.isfile(os.path.join(tmpdir, 'compiler.html')))
        ...
        for page_file in [
            ...
            with open(path, 'r', encoding='utf-8') as f:
                content = f.read()
            check(f'{page_file} has site header', 'site-header' in content)
...
def test_integration():
    ...
    check(
        'Site command uses site_generator',
    ...
    check('Registry can access BUILTIN_REGISTRY', hasattr(GitHubRegistry, 'search'))
...
def test_toml_parser():
    ...
    from epl.package_manager import _dump_toml, _parse_toml, _toml_value

    # T1: Parse simple key-value
    toml_text = 'name = "hello"\nversion = "1.0.0"'
    result = _parse_toml(toml_text)
    check(
        'Parse simple strings', result.get('name') == 'hello' and result.get('version') == '1.0.0'
    )
    ...
    check(
        ...
        result5.get('project', {}).get('name') == 'test'
        and result5.get('project', {}).get('version') == '2.0.0',
    )
    ...
    check(
        'Parse nested tables',
        result6.get('project', {}).get('name') == 'app'
        and result6.get('dependencies', {}).get('epl-math') == '^1.0.0',
    )
    ...
    check(
        'Parse inline tables',
        result7.get('server', {}).get('host') == 'localhost'
        and result7.get('server', {}).get('port') == 8080,
    )
...
"""
    ...
    check(
        'Full epl.toml parse',
        result13.get('project', {}).get('name') == 'my-app'
        and result13.get('project', {}).get('version') == '1.2.3'
        and result13.get('dependencies', {}).get('epl-math') == '^1.0.0'
        and result13.get('dev-dependencies', {}).get('epl-testing') == '^1.0.0'
        and result13.get('scripts', {}).get('start') == 'epl main.epl'
        and result13.get('project', {}).get('keywords') == ['test', 'demo'],
    )
    ...
    data = {
        'project': {'name': 'roundtrip', 'version': '1.0.0'},
        'dependencies': {'epl-math': '^1.0'},
    }
    dumped = _dump_toml(data)
    reparsed = _parse_toml(dumped)
    check(
        'TOML dump/parse round-trip',
        reparsed.get('project', {}).get('name') == 'roundtrip'
        and reparsed.get('dependencies', {}).get('epl-math') == '^1.0',
    )
    ...
    check(
        'Array of tables',
        isinstance(result17.get('servers'), list)
        and len(result17['servers']) == 2
        and result17['servers'][0].get('host') == 'alpha'
        and result17['servers'][1].get('host') == 'beta',
    )
...
def test_toml_manifest():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            result = create_manifest(
                name='toml-test', version='1.0.0', description='TOML Test Project', fmt='toml'
            )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            create_manifest(
                name='migrate-test', version='3.0.0', description='Migration test', fmt='json'
            )
    ...
    check(
        'Tool section preserved', m2.get('tool', {}).get('linter', {}).get('max-line-length') == 100
    )
...
def test_dependency_management():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            check(
                'print_dependency_tree runs', 'print-tree' in output or 'No dependencies' in output
            )
        finally:
            os.chdir(old_cwd)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            check(
                'audit_packages clean project', result.get('ok') == 0 and result.get('errors') == []
            )
        finally:
            os.chdir(old_cwd)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            loaded = load_manifest('.')
            check(
                'add_dependency updates manifest',
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            loaded = load_manifest('.')
            check(
                'remove_dependency removes from manifest',
...
def test_semver_production():
    ...
    check(
        'SemVer.tilde_compatible blocks 1.3.0', not v_base.tilde_compatible(SemVer.parse('1.3.0'))
    )
...
def test_install_pipeline():
    ...
    check(
        'Validate https URL',
        _validate_url('https://example.com/pkg.zip') == 'https://example.com/pkg.zip',
    )
    try:
...
def test_publish_workflow():
    ...
    from epl.package_manager import create_manifest, pack_package, publish_package, validate_package
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            create_manifest(
                name='pub-test', version='1.0.0', description='Test package', fmt='toml'
            )
            with open('main.epl', 'w') as f:
                f.write('Print "Hello"')
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            check(
                ...
                not result['valid']
                or any('description' in e.lower() for e in result.get('errors', [])),
            )
        finally:
            os.chdir(old_cwd)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        check(
            'validate error mentions manifest',
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            result = validate_package('.')
            check(
                'validate missing entry point', any('entry' in e.lower() for e in result['errors'])
            )
        finally:
            os.chdir(old_cwd)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        os.chdir(tmp)
        try:
            create_manifest(name='pack-test', version='1.0.0', description='Pack test', fmt='toml')
            with open('main.epl', 'w') as f:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            create_manifest(
                name='contents-test', version='2.0.0', description='Contents test', fmt='toml'
            )
            with open('main.epl', 'w') as f:
            ...
            if archive:
                ...
                with _zf.ZipFile(archive, 'r') as zf:
                    names = zf.namelist()
                    check('Archive contains manifest', 'epl.toml' in names or 'epl.json' in names)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            create_manifest(
                name='local-pub', version='1.0.0', description='Local publish test', fmt='toml'
            )
            with open('main.epl', 'w') as f:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            create_manifest(
                name='legacy-warn', version='1.0.0', description='Legacy test', fmt='json'
            )
            with open('main.epl', 'w') as f:
                f.write('Print "Legacy"')
            result = validate_package('.')
            check(
                'validate warns on JSON manifest',
                any(
                    'toml' in w.lower() or 'legacy' in w.lower() for w in result.get('warnings', [])
                ),
            )
        finally:
            os.chdir(old_cwd)
...
def test_registry_production():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        cache = RegistryCache(path=cache_path)
        check(
            'Fresh cache not stale (or stale if never written)', True
    ...
    check('Search finds epl-math', any(r['name'] == 'epl-math' for r in results))
    ...
    # T14: Score matching
    score = reg._score_match(
        'epl-math', {'description': 'Math functions', 'keywords': ['math']}, 'math', {'math'}
    )
    check('Score matching positive', score > 0)

    score_zero = reg._score_match(
        'epl-crypto',
        {'description': 'crypto', 'keywords': ['crypto']},
        'zznotfound',
        {'zznotfound'},
    )
...
def test_e2e_integration():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            check('E2E: package installed', os.path.isdir(os.path.join(PACKAGES_DIR, 'epl-math')))
            ...
            # Dependency tree
            tree = dependency_tree('.')
            check('E2E: tree shows epl-math', any(n['name'] == 'epl-math' for n in tree))

            # Audit
            audit = audit_packages('.')
            check(
                'E2E: audit passes', audit.get('ok', 0) >= 1 and len(audit.get('errors', [])) == 0
            )
            ...
            archive = pack_package('.')
            check(
                'E2E: pack created archive',
                archive is not None and os.path.exists(archive) if archive else False,
            )
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            add_dependency('epl-testing', '>=1.0.0', dev=True)
            m = load_manifest('.')
            check('E2E: dev-deps in manifest', 'epl-testing' in m.get('dev-dependencies', {}))
        finally:
            os.chdir(old_cwd)
