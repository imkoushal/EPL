...
def test_at_version_syntax():
    ...
    check(
        'install_package accepts name@version signature',
        callable(install_package)
        and install_package.__code__.co_varnames[:5]
        == ('name_or_url', 'version', 'save', 'local', 'project_path'),
    )
    ...
    for input_str, expected_name, expected_ver in test_cases:
        ...
        check(
            f"Parse '{input_str}' -> name='{parts[0]}', ver='{parts[1]}'",
...
def test_unified_naming():
    print('\n=== 2. Unified Package Name Resolution ===')
    from epl.package_manager import BUILTIN_REGISTRY, _resolve_package_name
    ...
    check(
        "'collections' resolves to 'epl-collections'",
        _resolve_package_name('collections') == 'epl-collections',
    )
...
def test_auto_save():
    ...
    try:
        ...
        manifest = load_manifest(tmpdir)
        check(
            "Auto-save doesn't overwrite existing dep",
            manifest.get('dependencies', {}).get('epl-math') == '^1.0.0',
        )
        ...
        manifest = load_manifest(tmpdir)
        check(
            'Auto-save handles multiple deps',
            'epl-math' in manifest.get('dependencies', {})
            and 'epl-http' in manifest.get('dependencies', {}),
        )
    finally:
    ...
    try:
        ...
        check(
            'Auto-save skips if no manifest', not os.path.exists(os.path.join(tmpdir2, 'epl.toml'))
        )
    finally:
...
def test_local_install():
    ...
    from epl.package_manager import _install_builtin_package, find_package_module
    ...
    try:
        ...
        check(
            'find_package_module finds local package', found is not None and 'epl_modules' in found
        )
        ...
        check(
            'Both packages in epl_modules/',
            os.path.isdir(os.path.join(tmpdir, 'epl_modules', 'epl-math'))
            and os.path.isdir(os.path.join(tmpdir, 'epl_modules', 'epl-json')),
        )
    ...
    try:
        ...
        check(
            'Default install not in epl_modules/',
...
def test_auto_install_import():
    print('\n=== 5. Auto-Install on Import ===')
    from epl.package_manager import PACKAGES_DIR, auto_install_package, find_package_module
...
def test_epl_new_toml():
    ...
    try:
        ...
        check(
            ...
            manifest is not None and manifest.get('name') == 'test-project',
        )
...
def test_uninstall_manifest():
    ...
    try:
        ...
        manifest = load_manifest(tmpdir)
        check(
            'Package installed and in manifest', 'epl-logging' in manifest.get('dependencies', {})
        )
        ...
        manifest = load_manifest(tmpdir)
        check(
            'Uninstall removes from manifest', 'epl-logging' not in manifest.get('dependencies', {})
        )
...
def test_package_resolution():
    ...
    try:
        ...
        check(
            'find_package_module finds single-file module',
...
def test_e2e_workflow():
    ...
    try:
        ...
        manifest = load_manifest(tmpdir)
        check(
            "E2E: no-save doesn't update manifest",
        ...
        manifest = load_manifest(tmpdir)
        check(
            'E2E: @version install saves correct version',
        ...
        manifest = load_manifest(tmpdir)
        check(
            'E2E: Shorthand name resolves and saves',
...
def test_manifest_format():
    ...
    try:
        ...
        check('create_manifest creates epl.toml', os.path.isfile(os.path.join(tmpdir, 'epl.toml')))

        # T2: load_manifest reads back correctly
        loaded = load_manifest(tmpdir)
        check(
            'load_manifest reads toml back', loaded is not None and loaded.get('name') == 'fmt-test'
        )
        ...
        check('save_manifest preserves deps', 'epl-math' in reloaded.get('dependencies', {}))
        ...
        check('TOML round-trip preserves deps', 'epl-math' in back.get('dependencies', {}))
...
def test_security():
    print('\n=== 15. Security and Validation ===')
    from epl.package_manager import _resolve_package_name, _sanitize_package_name
